...
class BrainDataset(AbstractDataset):
    def __init__(self,
        ...
        # opening data from hdf5/csv ...
        info_df = pd.read_csv(info, index_col=0, dtype={"key": "string", column: np.float32})
        self.keys = [l.strip() for l in Path(fkeys).open().readlines()]
...
class BrainPatchDataset(AbstractDataset):
    ...
    def __getitem__(self, i):
        ...
        # get valid indices of a random patch
        ...
        sample["data"] = np.squeeze(sample["data"], axis=0)
        sample["position"] = [(b.start, b.stop) for b in list(box)]
        return sample
